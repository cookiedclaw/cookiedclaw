 ...
 *
 * Wrapped in `runGateway()` so the CLI dispatcher in `gateway.ts` can
 * dynamic-import this module only when no subcommand was given — this
 * keeps `--version` / `--help` / `update` clean of env side effects
 * (bot.ts module load logs to stderr if no Telegram token is set, and
 * the GATEWAY_TOKEN check below exits the process).
 */
  ...
  async function checkForUpdate(): Promise<void> {
    ...
    try {
      ...
      if (latest && latest !== VERSION) {
        ...
        console.error(
          `[gateway] update available: v${VERSION} -> v${latest}`,
        );
      } else {
  ...
  async function notifyUpdateIfAvailable(server: McpServer): Promise<void> {
    ...
    try {
      await server.server.notification({
        ...
        params: {
          content: `cookiedclaw gateway update available: v${VERSION} → v${updateCache.latest}. Run \`cookiedclaw-gateway update\` to upgrade — fetches the latest binary for this platform, verifies sha256, swaps in place, and restarts the systemd unit if active.`,
          meta: {
